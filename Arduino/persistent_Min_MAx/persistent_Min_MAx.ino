 ...
 //Serial.println(a);
  if (a==1){
   ...
   max_T=temp;
    ...
    if (temp >= max_T){
    max_T = temp;
    EEPROM_writeAnything(0, max_T); // Byte 0 für Max
    }
    if (temp < min_T){
    min_T = temp;
    EEPROM_writeAnything(1, min_T);} // Byte 0 für min 
    else {
    ;
    }
...
float save =0; 
  ...
  Serial.println(save);
  EEPROM_readAnything(0, save);
  Serial.print("MAX: ");
  Serial.println(save);
  Serial.print("Min: ");
  EEPROM_readAnything(1, save);
  Serial.println(save);
